...
.PHONY: build
build: dist/usrsctp $(tests)

$(tests): dist/tests/% : dist/usrsctp $(test-srcfiles) package.yaml stack.yaml
	mkdir -p dist/tests
	cp "$(shell stack ${stack_args} path --dist-dir)/build/$(notdir $@)/$(notdir $@)" dist/tests/

dist/usrsctp: $(srcfiles) $(test-srcfiles) package.yaml stack.yaml
	rm -f *.cabal
